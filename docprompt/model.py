...
class EncoderWrapper(torch.nn.Module):
    ...
    def forward(self, input_ids=None, attention_mask=None, **kwargs,):
        ...
        # outputs may be a ModelOutput or a tuple. Normalize and return a BaseModelOutput
        last_hidden = outputs[0].view(bsz, self.n_passages*passage_length, -1)
        hidden_states = None
        attentions = None
        # try to extract hidden_states and attentions if present
        try:
            # If outputs is a ModelOutput-like object with attributes
            hidden_states = outputs.hidden_states if hasattr(outputs, 'hidden_states') else (outputs[1] if len(outputs) > 1 else None)
            attentions = outputs.attentions if hasattr(outputs, 'attentions') else (outputs[2] if len(outputs) > 2 else None)
        except Exception:
            hidden_states = None
            attentions = None

        # Import BaseModelOutput lazily to avoid hard dependency issues
        try:
            from transformers.modeling_outputs import BaseModelOutput
            return BaseModelOutput(last_hidden_state=last_hidden, hidden_states=hidden_states, attentions=attentions)
        except Exception:
            # Fallback to tuple output for older transformer versions
            rest = ()
            if hidden_states is not None:
                rest = rest + (hidden_states,)
            if attentions is not None:
                rest = rest + (attentions,)
